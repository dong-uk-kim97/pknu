...
########################################################################
        ...
        self.btn2.clicked.connect(self.input_rtsp)
...
class Thread(QThread):
    ...
    def run(self):
        con = sqlite3.connect('C:\\study\\20240219\\rtsp.db')
        cur = con.cursor()
        cur.execute(f'SELECT path FROM Rtsp WHERE name="{self.text}"')
        path = cur.fetchone()
        # print(path[0])
