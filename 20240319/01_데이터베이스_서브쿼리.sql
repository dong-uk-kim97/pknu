...
/*
<서브쿼리 규칙>
	- In()을 이용하는 경우 : 다중행에 단일 컬럼 형태만 조회 가능
								  : Where 절(조건절) 내에만 사용 가능 
	- = 조건을 이용하는 경우 : 단일 행에 단일 컬럼 형태만 조회 가능
									 : 컬럼명 또는 where(조건절) 내에 사용 가능
	- From 뒤에 테이블로 이용하는 경우 : 다중행에 다중 컬럼 형태 모두 사용 가능
*/
